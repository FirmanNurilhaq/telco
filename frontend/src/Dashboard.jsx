...
// Chart styles — updated dynamically via hook below
const AT = { fill:'#9ca3af', fontSize:10 };


function ChartCard({ id, title, desc, children, className='' }) {
  return (
    <div id={id} className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-transparent rounded-xl p-4 shadow ${className}`}>
  ...
  return (
    <button onClick={onClick}
      className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors whitespace-nowrap
        ${active ? 'bg-blue-600 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
      {label}
    </button>
  );
}
...
function DistTooltip({ active, payload, label, isDark }) {
  if (!active || !payload?.length) return null;
  const d = payload[0].payload;
  return (
    <div className={`border rounded px-3 py-2 text-xs ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`}>
      <p className={`font-semibold mb-1 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{label}</p>
...
export default function Dashboard() {
  ...
  const CS = { background: isDark ? '#1f2937' : '#ffffff', border: isDark ? 'none' : '1px solid #e5e7eb', color: isDark ? '#f3f4f6' : '#111827', fontSize:12, borderRadius:6 };
  const GS = isDark ? '#374151' : '#e5e7eb';
  const AT_THEMED = { fill: isDark ? '#9ca3af' : '#6b7280', fontSize:10 };
  ...
  if(loading) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6">
      <div className="max-w-screen-2xl mx-auto">
        <div className="h-8 bg-gray-200 dark:bg-gray-800 rounded w-64 mb-6 animate-pulse"/>
  ...
  if(error) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center flex-col gap-2">
      ...
      <button onClick={loadAll} className="mt-2 text-xs text-blue-500 dark:text-blue-400 hover:text-blue-400">{LABELS.cobLagi}</button>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      {/* Bilah atas */}
      <div className="sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 py-3">
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between gap-3 flex-wrap">
          <div className="min-w-0">
            <p className="text-sm sm:text-base font-bold text-gray-900 dark:text-white leading-tight truncate">Telkomsel NW RF National Tracker</p>
      ...
      <div className="p-4 sm:p-6 max-w-screen-2xl mx-auto">
        ...
        {/* Kartu KPI */}
        <div id="viz-kpi" className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 mb-4">
        ...
        {s&&(
          <div className={`rounded-lg px-4 py-2 mb-4 text-xs flex items-center gap-3 flex-wrap ${
            s.data_valid===false?'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300 border border-red-300 dark:border-red-700'
            :s.data_valid===true?'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800'
            :'bg-gray-100 dark:bg-gray-800 text-gray-500 border border-gray-200 dark:border-transparent'}`}>
        ...
        {activeTab===0&&(
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
              <ChartCard id="viz-regional-bar" title={LABELS.progresPerRegional} desc={LABELS.descProgresRegional}>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={byRegion} margin={{top:0,right:10,left:-10,bottom:70}}>
                    <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                    <XAxis dataKey="region" tick={AT_THEMED} angle={-40} textAnchor="end" interval={0}/>
                    <YAxis tick={AT_THEMED}/>
              ...
              <ChartCard id="viz-line-plan-actual" title={LABELS.rencanaVsRealisasi} desc={LABELS.descRencanaRealisasi}>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={timeline} margin={{top:0,right:10,left:-10,bottom:70}}>
                    <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                    <XAxis dataKey="month" tick={AT_THEMED} angle={-40} textAnchor="end" interval={0}/>
                    <YAxis tick={AT_THEMED}/>
        ...
        {activeTab===1&&(
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
              <ChartCard id="viz-vendor" title={LABELS.performaVendor} desc={LABELS.descVendor}>
                <div className="overflow-x-auto">
                  <ResponsiveContainer width="100%" height={320} minWidth={300}>
                    <BarChart data={vendors} layout="vertical" margin={{top:0,right:40,left:110,bottom:0}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                      <XAxis type="number" tick={AT_THEMED}/>
                      <YAxis dataKey="vendor" type="category" tick={AT_THEMED} width={110}/>
                      <Tooltip contentStyle={CS} formatter={(v,n)=>[v,STATUS_ID[n]||n]}/>
                      <Legend wrapperStyle={{fontSize:11}} formatter={n=>STATUS_ID[n]||n}/>
              ...
              <ChartCard id="viz-pic" title={LABELS.bebanKerjaPIC} desc={LABELS.descPIC}>
                <div className="overflow-x-auto">
                  <ResponsiveContainer width="100%" height={320} minWidth={300}>
                    <BarChart data={pics.slice(0,15)} layout="vertical" margin={{top:0,right:30,left:110,bottom:0}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                      <XAxis type="number" tick={AT_THEMED}/>
                      <YAxis dataKey="pic" type="category" tick={AT_THEMED} width={110}/>
            ...
            <ChartCard id="viz-distribution" title={LABELS.distribusiProgres} desc={LABELS.descDistribusi}>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={dist} margin={{top:10,right:20,left:-10,bottom:0}}>
                  <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                  <XAxis dataKey="label" tick={{...AT_THEMED,fontSize:11}}/>
                  <YAxis tick={AT_THEMED}/>
                  <Tooltip content={<DistTooltip isDark={isDark}/>}/>
        ...
        {activeTab===2&&(
          <>
            <ChartCard id="viz-issues" title={LABELS.penyebabKeterlambatan} desc={LABELS.descKeterlambatan} className="mb-4">
              <div className="overflow-x-auto">
                <ResponsiveContainer width="100%" height={280} minWidth={300}>
                  <BarChart data={delayKw} layout="vertical" margin={{top:0,right:40,left:130,bottom:0}}>
                    <CartesianGrid strokeDasharray="3 3" stroke={GS}/>
                    <XAxis type="number" tick={AT_THEMED}/>
                    <YAxis dataKey="reason" type="category" tick={AT_THEMED} width={130}/>
                    <Tooltip contentStyle={CS}/>
            ...
            <div>
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                ...
                {hasActiveFilter&&<span className="text-xs text-blue-500 dark:text-blue-400 ml-2">{LABELS.difilter}</span>}
              </p>
              {filteredRows.length===0
                ?<EmptyState message={LABELS.tidakAdaFilter}/>
                :<div id="viz-table"><DataTable rows={filteredRows}/></div>}
            </div>
          </>
        )}
        ...
        {activeTab===3&&(
          <div className="flex flex-col gap-6">
            ...
            <div id="viz-map-site" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-transparent rounded-xl p-4 shadow w-full">
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-0.5">{LABELS.petaSite}</p>
              ...
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                <span className="text-xs text-gray-400">{filteredPoints.length} {LABELS.siteDitampilkan}</span>
                {hasActiveFilter&&<span className="text-xs text-blue-500 dark:text-blue-400">{LABELS.difilter}</span>}
              </div>
            </div>

            {/* Peta Regional — lebar penuh */}
            <div id="viz-map-region" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-transparent rounded-xl p-4 shadow w-full">
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-0.5">{LABELS.petaRegion}</p>
